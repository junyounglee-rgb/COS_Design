@echo off
title Cherry Pick Propagator - Install
cd /d "%~dp0"
echo.
echo  =========================================
echo   Cherry Pick Propagator - Install
echo  =========================================
echo.
set "UV_EXE="
where uv >nul 2>&1
...
echo  [1/3] Installing package manager...
...
if not defined UV_EXE (echo. & echo  [ERROR] uv installation failed. & echo. & pause & exit /b 1)
echo  [1/3] Done!
goto :step2
:uv_ok
echo  [1/3] uv found. OK
:step2
if exist ".venv\Scripts\python.exe" (echo  [2/3] Python environment exists. OK & goto :step3)
echo  [2/3] Creating Python 3.12 environment...
"%UV_EXE%" python install 3.12 >nul 2>&1
"%UV_EXE%" venv .venv --python 3.12
if %errorlevel% neq 0 (echo. & echo  [ERROR] Failed to create Python environment. & pause & exit /b 1)
echo  [2/3] Done!
:step3
if exist ".venv\Lib\site-packages\streamlit" (echo  [3/3] Packages already installed. OK & goto :done)
echo  [3/3] Installing packages...
"%UV_EXE%" pip install -r requirements.txt --quiet 2>nul
if %errorlevel% neq 0 (echo. & echo  [ERROR] Package installation failed. & pause & exit /b 1)
echo  [3/3] Done!
:done
echo.
echo  =========================================
echo   Installation complete!
echo   Run run_cherry_pick.bat to start.
echo  =========================================
echo.
pause
