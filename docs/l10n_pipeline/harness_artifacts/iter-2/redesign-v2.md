...
| M2 "3초 수렴" | 사실상 포기 | Phase 2 Slack 커맨드 + 자동 PR로 구조화 | handoff-v1 (C) |
...
| P2 비용 역전 | △ → O (D 보강 시) | 단독 A는 여전히 Excel 5단계. Slack 커맨드(Phase 2)로 3초 수렴 달성. |
...
- **(3) 개발공수 4~6주 적합도** (1~2명 기준)
  - A: 프록시 Lambda(1주) + 권한 재설계(1주) + validator(0.5주) + Slack 커맨드(0.5주) = 3주 → 9
...
- **선정 이유**:
  ...
  - M2 "3초 수렴"은 Phase 2 Slack 커맨드(ko 오타 신고 → 자동 PR 30초 SLA)로 달성
...
### Phase 2 — **Validator 3종 + pb gitignore + CI pb 생성 + Slack 커맨드** (Week 2~3)
...
- **작업 (validator 3종, handoff-v1 A-1 핵심 반영)**:
  ...
  - **M2 Slack 커맨드 추가** (handoff-v1 (C) 핵심): `/ko-typo <str_id> <수정후>` 명령 → 기획 담당자 코드오너 자동 승인 30초 SLA → `translations.xlsx` 자동 PR → CI 머지. 공식 경로 종단 시간 3초 수렴 (spec v2 M2 달성).
...
- **Exit**:
  ...
  - Slack `/ko-typo` 커맨드 종단 시간 측정 < 3초, 자동 PR 생성 확인

- **롤백 포인트**: `.gitignore` 되돌리기 + 이전 pb 파일 백업 복원. validator 비활성화 토글. Slack 커맨드 off. BFG는 적용 전이므로 롤백 가능.
...
| 2 | W2~3 | **Validator 3종 + pb gitignore + CI pb + Slack 커맨드** | `.gitignore` 되돌리기 |
...
| M2 3초 수렴 | Slack 커맨드 | Phase 2 작업 "Slack `/ko-typo` 커맨드 → 자동 PR 30초 SLA" + Exit "종단 시간 측정 < 3초" |
...
| 개발 리소스 1~2명 × 4~6주 | O | Phase 0~4 총 5주, 1~2명 (프록시 Lambda 1주 + 권한 재설계 1주 + validator 0.5주 + Slack 커맨드 0.5주 + cron 1주 + 태깅 자동화 1주 = 5주) |
