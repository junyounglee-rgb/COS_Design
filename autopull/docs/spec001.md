...
## 개선 7차 (2026-04-24, **Pull 성능 최적화**: up-to-date 13.5× 가속)

| 항목 | 내용 |
|------|------|
| 작성일 | 2026-04-24 |
| 대상 파일 | `autopull.py` |
| 상태 | 구현 완료 (사용자 환경 실측 검증 완료) |
| 측정 환경 | 브랜치 3개 × 저장소 3개 (cos-data/common/client), 전 브랜치 이미 최신 |

### 배경

사용자 피드백: **"이미 최신인 브랜치(완료)를 처리하는게 너무 오래 걸려."**

실제 Pull 화면에서 `git pull origin branch` 한 번에 `cos-client`는 최대 14초까지 소요 → 브랜치 3개 × 저장소 3개 = **9회 반복 pull → 합산 ~42초**.

### 프로파일링 (Before 실측)

| 저장소 | 브랜치당 `git pull origin branch` 소요 | 9회 합계 |
|---|---|---|
| cos-common | 2.4–2.5s | ~7.4s |
| cos-data | 1.5–1.9s | ~5.1s |
| cos-client | **5–14s** (워킹트리 파일 스캔 크게 소요) | **~28.8s** |
| **합계** | — | **~41.82s** |

**병목 원인**:
1. `git pull`은 내부에서 `fetch`를 호출 → 매 호출마다 원격 네트워크 round-trip
2. cos-client처럼 대규모 저장소는 fetch 이후에도 내부 ref 처리에 추가 소요
3. 원래 `_process_branch`는 저장소별 직렬 실행 → wall-clock에 모든 repo 시간 누적

### 최적화 3단 구성

#### Fix 1 — Phase 1 병렬 fetch (`run()` 시작부)

`Puller.run()`이 본격 루프 진입 전 모든 저장소를 **한 번에** 병렬 fetch (status check와 동일 패턴 재사용).
...
이후 루프에서는 **로컬 ref만 조회** → 네트워크 호출 완전 제거.

#### Fix 2 — behind=0 스킵 (`_pull_repo_branch` 신규)

`fetch --all --prune` 이후엔 `git rev-list --count branch..origin/branch`로 정확한 behind 카운트를 즉시 얻음. behind=0이면 `git checkout`/`git merge`조차 실행할 필요 없음.

```python
def _pull_repo_branch(self, repo_path, branch) -> tuple[name, kind, err, log_line]:
    behind = _check_repo_behind(repo_path, branch)
    if behind is None:
        return (name, "skip", None, "브랜치 없음 - 건너뜀")
    if behind == 0:
        return (name, "uptodate", None, "✓ 이미 최신")  # checkout/merge 완전 스킵
    # behind > 0 에만 checkout + merge --ff-only
    ...
```

#### Fix 3 — `git pull` → `git merge --ff-only origin/branch`

`git pull`은 항상 fetch+merge를 수행하지만, Phase 1에서 이미 fetch 완료이므로 **merge만** 실행하면 충분.

```python
# Before: 네트워크 호출 내포
r = run_git(["pull", "origin", branch], cwd=repo_path)

# After: 로컬 merge만 (네트워크 없음)
r = run_git(["merge", "--ff-only", f"origin/{branch}"], cwd=repo_path)
```

`--ff-only`는 로컬 divergence 감지 시 자동 merge commit을 만들지 않고 실패 → 의도치 않은 merge commit 방지 부가효과.

#### Fix 4 — `_process_branch` 내부 저장소 병렬화

한 브랜치 내에서 여러 저장소의 checkout+merge는 서로 독립적이므로 `ThreadPoolExecutor`로 병렬 실행. 로그는 완료 순서가 아닌 **고정 순서(sibling → main)**로 emit 하여 가독성 유지.

```python
ordered_targets = valid_siblings + [self.repo_root]
with ThreadPoolExecutor(max_workers=len(ordered_targets)) as ex:
    futures = {ex.submit(self._pull_repo_branch, p, branch): p for p in ordered_targets}
    for fut in as_completed(futures):
        outcomes[futures[fut]] = fut.result()
# 고정 순서로 로그 출력
for path in ordered_targets:
    self.log(outcomes[path][3])
```

### 실측 결과 (사용자 환경, 전 브랜치 이미 최신)

| 지표 | Before | After | 개선 |
|---|---|---|---|
| **전체 Pull 소요** | **41.82s** | **3.09s** | **13.5×** |
| `git pull` 개별 호출 | 1.5–14s × 9회 | 0회 (모두 스킵) | ∞ |
| fetch 네트워크 호출 | 9회 (각 pull 내부) | **1회** (phase 1 병렬) | 9× 감소 |
| 저장소 직렬 순회 | 3 repos × sequential | `ThreadPool(3)` 병렬 | 3× |

### 의미 동등성 (behavior compatibility)

| 시나리오 | Before | After |
|---|---|---|
| behind=0 (모든 저장소 최신) | "Already up to date." 로그 | "✓ 이미 최신" 로그 (의미 동일) |
| behind>0 (FF 가능) | `git pull origin branch` → "Updating abc..def Fast-forward" | `git merge --ff-only origin/branch` → 동일 Fast-forward |
| 로컬 divergence (local commit 존재) | **config에 따라 자동 merge commit 생성** (위험) | **`--ff-only` 실패 → 사용자에게 알림** (안전) |
| 원격 브랜치 없음 | checkout 실패 → 해당 repo skip | 로컬 ref 조회로 판정 → 명시적 "브랜치 없음" 로그 |

**Behavior change**: `--ff-only`는 divergence가 있으면 merge commit 대신 실패를 반환한다. 이는 **auto pull** 도구의 본래 의도(원격 상태로 catch-up)에 부합하며, 사용자의 로컬 커밋 손실 위험을 차단함.

### 보조 변경

- `Puller.__init__`에 `_valid_siblings_cache: list[str]` 초기화 (run() phase 1에서 세팅, _process_branch에서 참조)
- `from concurrent.futures import ThreadPoolExecutor, as_completed` import 추가

### QA 검증

- `ast.parse(autopull.py)` + `ast.parse(app_autopull.py)` → **OK**
- 사용자 환경 실측:
  - 최적화 전(재현 스크립트): 41.82s, 전 브랜치 "Already up to date."
  - 최적화 후(`Puller.run()`): 3.09s, 전 브랜치 `uptodate` kind → `result.success=True`
- 로그 가독성 확인: sibling → main 순서로 고정, 병렬 완료 비결정성 제거

**qa-tool 에이전트 검증 (PASS 11 / WARN 5 / FAIL 0)**

| TC | 판정 | 비고 |
|---|---|---|
| TC-P1 Python 문법 | PASS | |
| TC-P2 `_valid_siblings_cache` AttributeError 방지 | PASS | `__init__`에서 `[]` 초기화 |
| TC-P3 5종 kind 분기 | PASS | skip/uptodate/merged/checkout_fail/merge_fail 모두 처리 |
| TC-P4 ThreadPool thread-safety | PASS | worker별 독립 repo, `queue.Queue` 내부 Lock |
| TC-P5 로그 순서 안정성 | PASS | `as_completed` 수집 후 `ordered_targets` 순서로 emit |
| TC-B1 `"Already up to date." → "✓ 이미 최신"` | WARN P3 | 의도된 UI 개선 (유지) |
| TC-B2 divergence 에러 메시지 품질 | PASS | |
| TC-B3 원격 브랜치 없음 → `error_stage` | **WARN P2 → Fix 반영** | `"branch missing"` → **`"git checkout (원격 브랜치 없음)"`** |
| TC-B4 checkout 실패 | PASS | |
| TC-B5 merge 실패 → `error_stage` | **WARN P3 → Fix 반영** | `"git merge --ff-only"` → **`"git pull (ff-only)"`** |
| EC-1 sibling 비어있음 | PASS | |
| EC-2 sibling 경로 없음 | **WARN P3 → Fix 반영** | Phase 1에서 `"[경고] sibling 경로 없음 - 건너뜀: {s}"` 로그 출력 |
| EC-3 브랜치 빈 경우 | PASS | |
| EC-4 fetch 실패 시 stale ref 위험 | WARN P3 | 기존 경고 로그로 인지 가능 (추가 fix는 비용 대비 효과 낮아 보류) |
| EC-5 `_valid_siblings_cache` invariant | PASS | |
| 회귀: 인터페이스 호환 (6항목) | PASS | Puller/PullerThread/BranchResult 시그니처 동일 |

### 잔존 한계

- behind>0 케이스의 wall-clock은 대형 repo의 checkout+merge 시간에 여전히 비례 (병렬화로 완화하나 완전 제거 불가)
- 로컬 divergence 발생 시 `--ff-only` 실패 → 사용자가 수동으로 해결해야 함 (의도된 안전장치)
- EC-4: fetch 실패한 저장소에 대해 `_check_repo_behind`가 stale ref 기반으로 오판 가능. 경고 로그로 인지는 가능하나 자동화 보강은 후속 과제
